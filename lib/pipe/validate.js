...
export const normalizePipeArguments = (destination, from, {source, sourcePromise, stdioStreamsGroups, options}) => {
	...
	return {source, sourcePromise, sourceStream, destinationStream};
};

const getDestinationStream = destination => {
	try {
		if (!isExecaChildProcess(destination)) {
			throw new TypeError(`The first argument must be an Execa child process: ${destination}`);
		}
