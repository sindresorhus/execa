...
const create$ = options => {
	function $(templatesOrOptions, ...expressions) {
		...
		return scriptWithPipe(childProcess);
	}
...
const scriptWithPipe = returnValue => {
	const originalPipe = returnValue.pipe.bind(returnValue);
	const pipeMethod = (...args) => scriptWithPipe(originalPipe(...args));
	returnValue.pipe = scriptPipe.bind(undefined, pipeMethod, {});
	return returnValue;
};

const scriptPipe = (pipeMethod, options, firstArgument, ...args) => {
	if (PROCESS_OPTIONS.has(firstArgument)) {
		if (Object.keys(options).length > 0) {
			throw new TypeError('Please use .pipe(options)`command` or .pipe($(options)`command`) instead of .pipe(options)($`command`).');
		}

		return pipeMethod(firstArgument, ...args);
	}

	if (isPlainObject(firstArgument)) {
		return scriptPipe.bind(undefined, pipeMethod, {...options, ...firstArgument});
	}
	...
	return pipeMethod(childProcess, options);
};
