...
const getDuplexType = ({duplex}, optionName) => {
	if (!isDuplexStream(duplex, {checkOpen: false})) {
		throw new TypeError(`The \`${optionName}.duplex\` option must be a Duplex stream.`);
	}
...
export const isGeneratorOptions = value => isPlainObj(value)
	&& (value.transform !== undefined || value.final !== undefined);

const isDuplexOptions = value => isPlainObj(value)
	&& Object.keys(value).length === 1
	&& value.duplex !== undefined;
