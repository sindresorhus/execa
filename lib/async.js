...
export const execa = (rawFile, rawArgs, rawOptions) => {
	const {file, args, command, escapedCommand, verboseInfo, options, stdioStreamsGroups, stdioState} = handleAsyncArguments(rawFile, rawArgs, rawOptions);
	const {spawned, promise} = spawnProcessAsync({file, args, options, verboseInfo, command, escapedCommand, stdioStreamsGroups, stdioState});
...
const handleAsyncArguments = (rawFile, rawArgs, rawOptions) => {
	...
	try {
		...
		return {file, args, command, escapedCommand, verboseInfo, options, stdioStreamsGroups, stdioState};
	} catch (error) {
...
const spawnProcessAsync = ({file, args, options, verboseInfo, command, escapedCommand, stdioStreamsGroups, stdioState}) => {
	let spawned;
	try {
		spawned = spawn(file, args, options);
	} catch (error) {
		return handleEarlyError({error, command, escapedCommand, stdioStreamsGroups, options, verboseInfo});
	}
