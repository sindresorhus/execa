...
import {stdioHasIterableStdin} from './stdio.js';

export const validateInputOptions = ({input, inputFile}, stdio) => {
	if (input !== undefined && inputFile !== undefined) {
		throw new TypeError('The `input` and `inputFile` options cannot be both set.');
	}

	if (stdioHasIterableStdin(stdio)) {
		if (input !== undefined) {
...
export const handleInputSync = ({input, inputFile}, stdio) => {
	if (stdioHasIterableStdin(stdio)) {
...
export const handleInput = (spawned, {input, inputFile}, stdio) => {
	if (stdioHasIterableStdin(stdio)) {
		Readable.from(stdio[0]).pipe(spawned.stdin);
		return;
	}
