...
test('Prints command, NODE_DEBUG=execa + "inherit"', async t => {
	...
	t.deepEqual(getNormalizedLines(all), [
		`${testTimestamp} $ node -e ${QUOTE}console.error(1)${QUOTE}`,
		'1',
		`${testTimestamp} $ node -e ${QUOTE}process.exit(2)${QUOTE}`,
	]);
});

test('NODE_DEBUG=execa changes verbose default value to true', async t => {
...
const testDebugEnvPriority = async (t, execaMethod) => {
	const {stderr} = await execaMethod('noop.js', [foobarString], {verbose: false}, {env: {NODE_DEBUG: 'execa'}});
	t.is(getCommandLine(stderr), undefined);
};
