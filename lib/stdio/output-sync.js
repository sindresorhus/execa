...
const transformOutputResultSync = ({result, fileDescriptors, fdNumber, state, isMaxBuffer, verboseInfo}, {buffer, encoding, lines, stripFinalNewline, maxBuffer}) => {
	...
	} = serializeChunks({chunks, objectMode, encoding, lines, stripFinalNewline});
...
const serializeChunks = ({chunks, objectMode, encoding, lines, stripFinalNewline}) => {
	...
	if (lines) {
		return {serializedResult, finalResult: splitLinesSync(serializedResult, !stripFinalNewline, objectMode)};
	}
