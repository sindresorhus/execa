...
export const throwOnEarlyDisconnect = (methodName, isSubprocess) => {
	throw new Error(`${getNamespaceName(isSubprocess)}${methodName}() could not complete: the ${getOtherProcessName(isSubprocess)} exited or disconnected.`);
};
...
export const handleSerializationError = ({error, isSubprocess, methodName, message}) => {
	if (isSerializationError(error)) {
		error.message = `${getNamespaceName(isSubprocess)}${methodName}()'s argument type is invalid: the message cannot be serialized: ${String(message)}.\n${error.message}`;
	}
};
