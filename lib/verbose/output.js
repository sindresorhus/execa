...
const shouldLogOutput = (stdioStreams, {encoding}, isSync, {verbose}) => verbose === 'full'
	&& !isSync
	&& ALLOWED_ENCODINGS.has(encoding)
...
// Only print text output, not binary
// eslint-disable-next-line unicorn/text-encoding-identifier-case
const ALLOWED_ENCODINGS = new Set(['utf8', 'utf-8']);
...
const verboseGenerator = function * ({stdioState: {spawned: {stdio}}, fdNumber, verboseInfo}, line) {
...
// When `childProcess.stdout|stderr.pipe()` is called, `verbose` becomes a noop.
...
//    For example, `childProcess.stdout.pipe(process.stdin)` would print each line twice.
//  - When chaining processes with `childProcess.pipe(otherProcess)`, only the last one should print its output.
