...
export const normalizePipeArguments = ({source, sourcePromise, stdioStreamsGroups, destinationOptions}, ...args) => {
	...
	const sourceOptions = SUBPROCESS_OPTIONS.get(source);
	const {
		destination,
		destinationStream,
		destinationError,
...
const getDestinationStream = (destinationOptions, ...args) => {
	try {
		...
		if (destinationStream === null) {
			throw new TypeError('The destination subprocess\'s stdin must be available. Please set its "stdin" option to "pipe".');
		}
...
const getDestination = (destinationOptions, firstArgument, ...args) => {
	if (Array.isArray(firstArgument)) {
		const destination = create$({...destinationOptions, ...PIPED_SUBPROCESS_OPTIONS})(firstArgument, ...args);
...
const getInvalidStdioOptionMessage = (fdNumber, from, sourceOptions) => {
	if (fdNumber === 'all' && !sourceOptions.all) {
		return 'The "all" option must be true to use `subprocess.pipe(destinationSubprocess, {from: "all"})`.';
	}
	...
	return `The \`${optionName}: ${serializeOptionValue(optionValue)}\` option is incompatible with using \`subprocess.pipe(destinationSubprocess${pipeOptions})\`.
